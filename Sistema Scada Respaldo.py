...
fig_line = go.Figure()
            ...
            # 2. DEFINICIÓN DE EJES FORZADA
            # Al definir el range, forzamos a Plotly a mantener el eje visible aunque no haya datos
            fig_line.update_layout(
                ...
                # El eje Y del Caudal (yaxis) es el que queremos asegurar
                yaxis5=dict(title=dict(text="<b>Nivel Tanque (m)</b>", font=dict(color="#00ffcc")), tickfont=dict(color="#00ffcc"), side="left", overlaying="y", anchor="free", position=0.00, showline=True, linecolor='white', linewidth=1.5, range=[0, 100]),
                yaxis=dict(title=dict(text="<b>Caudal (Lps)</b>", font=dict(color="#00d4ff")), tickfont=dict(color="#00d4ff"), side="left", anchor="free", position=0.07, showline=True, linecolor='white', linewidth=1.5, range=[0, 100]),
                yaxis2=dict(title=dict(text="<b>Presión (Kg/cm²)</b>", font=dict(color="#00ff00")), tickfont=dict(color="#00ff00"), side="right", overlaying="y", anchor="free", position=0.92, showline=True, linecolor='white', linewidth=1.5, range=[0, 50]),
                yaxis3=dict(title=dict(text="<b>Niveles Pozo (m)</b>", font=dict(color="#ff00b4")), tickfont=dict(color="#ff00b4"), side="right", overlaying="y", anchor="free", position=0.955, showline=True, linecolor='white', linewidth=1.5, range=[0, 300]),
                yaxis4=dict(title=dict(text="<b>Eléctricos (V / A)</b>", font=dict(color="#ff8000")), tickfont=dict(color="#ff8000"), side="right", overlaying="y", anchor="free", position=1.00, showline=True, linecolor='white', linewidth=1.5, rangemode="tozero", range=[0, 500])
            )
            ...
            if not df.empty:
                ...
                for d in fechas_lineas:
                    fig_line.add_vrect(x0=d - pd.Timedelta(hours=0.15), x1=d + pd.Timedelta(hours=0.15), fillcolor="gray", opacity=0.2, layer="below", line_width=0)
