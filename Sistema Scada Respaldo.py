...
# 9.6. RENDERIZADO DE POZOS EN EL MAPA PRINCIPAL ---------------------------------------------------------------------------------------------
    for id_p, info in mapa_pozos_dict.items():
        if ver_pozos:  # Si el checkbox está activo, dibujamos todo
            ...
            # --- CAPTURA DE DATOS ELÉCTRICOS ---
            v = [d(t) for t in info['voltajes_l']] if not is_st else [(0.0, "N/A")]*3
            a = [d(t) for t in info['amperajes_l']] if not is_st else [(0.0, "N/A")]*3

            # --- GENERACIÓN DEL GRÁFICO (REPARADO PARA EVITAR TYPEERROR) ---
            try:
                # Llamamos a la función con los argumentos que ya conoce
                fig_mini = generar_grafico_integral_7d(id_p, info)
                
                # Ajustamos dimensiones y estilo HUD después de la creación
                fig_mini.update_layout(
                    ...
                    paper_bgcolor='rgba(0,0,0,0)',
                    plot_bgcolor='rgba(0,0,0,0)',
                    xaxis=dict(showgrid=False, color='gray', tickfont=dict(size=8)),
                    yaxis=dict(showgrid=True, gridcolor='#222', color='gray', tickfont=dict(size=8))
                )
                
                # Conversión a imagen Base64 para el HTML de Folium
                import base64
                ...
                grafico_html = '<div style="color: #666; font-size: 10px; text-align: center; padding: 15px;">Gráfico de tendencia no disponible</div>'
            ...
            # --- CONSTRUCCIÓN DEL POPUP (SEGÚN POPUP CON GRAFICO.JPG) ---
            html_popup = f"""
                <div style="background: #000000; color: white; padding: 15px; border-radius: 10px; width: 360px; border: 2px solid {info['color_final']}; font-family: sans-serif;">
                    ...
                    <div style="display: flex; justify-content: space-between; font-size: 12px; margin-bottom: 3px;">
                        <span>💧 <b>Caudal: {q:.2f} L/s</b></span> <span style="color: #cead41; font-size: 9px;">{f_q}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 12px; margin-bottom: 10px;">
                        <span>🚀 <b>Presión: {p:.2f} kg</b></span> <span style="color: #cead41; font-size: 9px;">{f_p}</span>
                    </div>

                    <div style="font-size: 10px; color: #666; margin-bottom: 5px;">NIVELES</div>
                    <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 2px;">
                        <span>🔋 Nivel de Tanque: <b>{tanq:.2f} mts</b></span> <span style="font-size: 9px;">{f_t}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 2px;">
                        <span>📉 Nivel Dinámico: <b>{dinam:.2f} m</b></span> <span style="font-size: 9px;">{f_d}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 2px;">
                        <span>📏 Sumergencia: <b>{sumer:.2f} m</b></span> <span style="font-size: 9px;">{f_s}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 10px;">
                        <span>🏗️ Longitud Columna: <b>{col:.2f} m</b></span> <span style="font-size: 9px;">{f_col}</span>
                    </div>
                    ...
                    <table style="width: 100%; font-size: 10px; border-collapse: collapse; margin-bottom: 10px;">
                        <tr style="color: #00d4ff; text-align: left; border-bottom: 1px solid #333;">
                            ...
                            <th style="padding-bottom: 4px;">Voltaje / Act.</th>
                            <th style="padding-bottom: 4px;">Amp / Act.</th>
                        </tr>
                        <tr>
                            <td style="padding: 4px 0;">L1-L2</td>
                            <td><b>{v[0][0]:.1f}V</b> <small style="color:#cead41;">{v[0][1]}</small></td>
                            <td><b>{a[0][0]:.1f}A</b> <small style="color:#cead41;">{a[0][1]}</small></td>
                        </tr>
                        <tr>
                            <td style="padding: 4px 0;">L2-L3</td>
                            <td><b>{v[1][0]:.1f}V</b> <small style="color:#cead41;">{v[1][1]}</small></td>
                            <td><b>{a[1][0]:.1f}A</b> <small style="color:#cead41;">{a[1][1]}</small></td>
                        </tr>
                        <tr>
                            <td style="padding: 4px 0;">L1-L3</td>
                            <td><b>{v[2][0]:.1f}V</b> <small style="color:#cead41;">{v[2][1]}</small></td>
                            <td><b>{a[2][0]:.1f}A</b> <small style="color:#cead41;">{a[2][1]}</small></td>
                        </tr>
                    </table>

                    <div style="font-size: 10px; color: #666; margin-bottom: 5px;">HORARIOS DE OPERACIÓN</div>
                    <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 2px;">
                        <span>▶️ Arranque: <b>{formato_hora(h_arr_val)}</b></span> <span style="font-size: 9px;">{f_h_arr}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 10px;">
                        <span>⏹️ Paro: <b>{formato_hora(h_par_val)}</b></span> <span style="font-size: 9px;">{f_h_par}</span>
                    </div>
