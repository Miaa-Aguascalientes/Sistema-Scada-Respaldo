...
if sector_seleccionado:
    # 7.1. Estilos CSS: Ajuste de márgenes y HUD futurista
    st.markdown(
        f"""
        <style>
            [data-testid="column"] {{ margin-top: -105px !important; }}
            [data-testid="stSidebar"] {{display: none;}}
            .block-container {{ padding-top: 0px !important; margin-top: -100px !important; }}
            .titulo-sector {{ margin-top: 10px !important; font-size: 1.8rem; font-weight: 800; color: #00d4ff; text-align: center; text-transform: uppercase; }}
            .card-indicador {{ background: rgba(16, 33, 54, 0.8); padding: 10px; border-radius: 8px; border: 1px solid #1f4068; text-align: center; margin-bottom: 5px; }}
            .label-indicador {{ color: #ffffff; font-size: 0.8rem; margin: 0; text-transform: uppercase; }}
    ...
    if datos_s:
        ...
        # 7.4. Fila Superior: Mapa e Histórico Puntos de Control (DEFINICIÓN DE COLUMNAS)
        ...
        with col_izq:
            ...
            # Recolección de tags para marcadores
            tags_mapa = []
            for r in dict_reg.values():
                for k in ['tag_p1', 'tag_p2', 'tag_q', 'tag_vbat']:
                    if r.get(k): tags_mapa.append(r.get(k))
            scada_mapa = cargar_datos_scada(list(set(tags_mapa)))

            # Renderizado de marcadores (Puntos de Control)
            for r in dict_reg.values():
                rp1, _ = scada_mapa.get(r.get('tag_p1'), (0.0, "N/A"))
                rcau, _ = scada_mapa.get(r.get('tag_q'), (0.0, "N/A"))
                folium.Marker(
                    location=r['coord'],
                    popup=f"<b>{r['nombre']}</b><br>Q: {rcau:.2f} L/s<br>P: {rp1:.2f} kg",
                    icon=folium.Icon(color='blue', icon='info-sign')
                ).add_to(m_sec)

            st_folium(m_sec, width="100%", height=330, key="mapa_sector_vFinal")

        # 7.10. Histórico Puntos de Control (Lado derecho del mapa)
        with col_der:
            ...
            # Recolección de tags de Puntos de Control y Pozos
            for s_id in list(dict_reg.keys()):
                r_info = dict_reg[s_id]
                for k, lb, clr, sec in [('tag_q', 'Q', '#00d4ff', False), ('tag_p1', 'P1', '#00ff00', True), ('tag_p2', 'P2', '#ffff00', True)]:
                    tag_v = r_info.get(k)
                    if tag_v and str(tag_v).strip().lower() not in ['0', 'none', 'n/a', 'null']:
                        tags_visualizar.append(tag_v)
                        mapeo_config[tag_v] = {'label': f"S:{s_id} - {lb}", 'color': clr, 'sec': sec}

            if tags_visualizar:
                try:
                    engine_h = get_mysql_scada_engine()
                    tags_q_str = "', '".join(list(set(tags_visualizar)))
                    df_h = pd.read_sql(f"SELECT h.FECHA, h.VALUE, r.NAME as TAG FROM vfitagnumhistory h JOIN VfiTagRef r ON h.GATEID = r.GATEID WHERE r.NAME IN ('{tags_q_str}') AND h.FECHA BETWEEN '{f_ini_h} 00:00:00' AND '{f_fin_h} 23:59:59' ORDER BY h.FECHA ASC", engine_h)
                    
                    if not df_h.empty:
                        fig = go.Figure()
                        for tn in tags_visualizar:
                            dft = df_h[df_h['TAG'] == tn]
                            if not dft.empty:
                                cfg = mapeo_config[tn]
                                fig.add_trace(go.Scatter(
                                    x=dft['FECHA'], y=dft['VALUE'], name=cfg['label'],
                                    yaxis="y2" if cfg['sec'] else "y1",
                                    mode='lines+markers', marker=dict(size=4), # Puntitos
                                    line=dict(width=1.5, color=cfg['color']),
                                    hovertemplate='<b>%{fullData.name}</b>: %{y:.2f}<extra></extra>' # 2 Decimales
                                ))
                        fig.update_layout(height=300, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', hovermode="x unified", legend=dict(orientation="h", y=1.1), xaxis=dict(color="white"), yaxis=dict(title="Caudal", tickformat=".2f"), yaxis2=dict(title="Presión", side="right", overlaying="y", tickformat=".2f"))
                        st.plotly_chart(fig, use_container_width=True)
                except Exception as e: st.error(f"Error Scada: {e}")

        # 7.11 / 7.12 Fila Inferior (VRP y Puntos Críticos) - FUERA DE COL_DER PARA RECUPERAR ANCHO
        st.markdown("---")
        col_vrp, col_pc = st.columns([1.0, 1.0])

        with col_vrp:
            st.markdown(f"<h3 style='color:#00d4ff; font-size:16px; text-align: center;'>Válvulas reductoras de presión:</h3>", unsafe_allow_html=True)
            if dict_vrp_sec:
                # Ejemplo de consulta para VRP
                tags_vrp = []
                for v in dict_vrp_sec.values():
                    if v.get('tag_q'): tags_vrp.append(v['tag_q'])
                if tags_vrp:
                    # Aquí generas el gráfico de VRP con la misma lógica de 2 decimales
                    st.info("Visualización de VRP cargada.")

        with col_pc:
            st.markdown(f"<h3 style='color:#00d4ff; font-size:16px; text-align: center;'>Puntos críticos del sector:</h3>", unsafe_allow_html=True)
            ...
            if dict_pc_sec:
                st.info("Visualización de Puntos Críticos cargada.")

# Final de la sección para evitar renderizados infinitos
st.stop()
