...
""", unsafe_allow_html=True)
            ...
            with st.expander("📅 Análisis de volumen real", expanded=False):
                if tag_totalizado and tag_totalizado != 'N/A':
                    ...
                    if not df_h.empty:
                        res_meses = df_h.groupby(['anio', 'mes'])[['VALUE', 'FECHA']].first().reset_index()
                        res_meses = res_meses.sort_values(['anio', 'mes'])
                        res_meses['produccion_neta'] = res_meses['VALUE'].diff()
                        
                        res_meses['produccion_neta'] = res_meses['produccion_neta'].fillna(0)
                        ...
                        with col_g:
                            ...
                            for an in sorted(res_meses['anio'].unique()):
                                df_a = res_meses[res_meses['anio'] == an]
                        ...
                        with col_t:
                            pivot = res_meses.pivot(index='mes', columns='anio', values='produccion_neta')
                            pivot.index = [nombres_meses[m] for m in pivot.index]
                            st.dataframe(pivot.style.format("{:,.2f}"), use_container_width=True)
                    st.info("No se encontraron registros de volumen para el periodo seleccionado.")
