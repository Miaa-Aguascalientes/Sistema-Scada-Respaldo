...
with col_der: # Esta es tu línea 1557
    ...
    # ... (Mantener tu lógica de selección de fechas aquí) ...
            ...
            if not df_h.empty:
                ...
                for tag_name in tags_visualizar:
                    ...
                    if not df_tag.empty:
                        ...
                        # AQUÍ ESTÁ LA MAGIA: Puntitos y 2 Decimales
                        fig.add_trace(go.Scatter(
                            x=df_tag['FECHA'], 
                            y=df_tag['VALUE'], 
                            name=cfg['label'], 
                            yaxis="y2" if cfg['sec'] else "y1", 
                            mode='lines+markers', # <--- Puntitos
                            line=dict(width=1.5, color=cfg['color']),
                            marker=dict(size=4, symbol='circle'), # <--- Tamaño del punto
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(title="Caudal / Otros", color="white", tickformat=".2f"),
                    yaxis2=dict(title="Presión / Nivel", side="right", overlaying="y", color="white", showgrid=False, tickformat=".2f")
                )
                st.plotly_chart(fig, use_container_width=True)
        except Exception as e:
