    ...
    if sel_v_id:
        ...
        try:
            ...
            if not df_v.empty:
                ...
                # 1. Configuración de Trazados (Líneas)
                dq = df_v[df_v['TAG'] == v_info.get('tag_q')]
                if not dq.empty: 
                    fig_v.add_trace(go.Scatter(x=dq['FECHA'], y=dq['VALUE'], name="Caudal VRP (Lps)", line=dict(color='#00d4ff', width=2)))
                
                dp1 = df_v[df_v['TAG'] == v_info.get('tag_p1')]
                if not dp1.empty: 
                    fig_v.add_trace(go.Scatter(x=dp1['FECHA'], y=dp1['VALUE'], name="P. Entrada (kg/cm2)", yaxis="y2", line=dict(color='#ff00ff', width=2)))
                
                dp2 = df_v[df_v['TAG'] == v_info.get('tag_p2')]
                if not dp2.empty: 
                    fig_v.add_trace(go.Scatter(x=dp2['FECHA'], y=dp2['VALUE'], name="P. Salida (kg/cm2)", yaxis="y2", line=dict(color='#00ff00', width=2)))
