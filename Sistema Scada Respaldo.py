...
if isinstance(df_incidencias, pd.DataFrame) and not df_incidencias.empty:
    ...
    for index, row in df_actual.iterrows():
        ...
        titulo = f"{ind} **Pozo: {row.get('NUM_POZO', 'N/A')}** | Inicio: {row['FECHA_HORA_INICIO'].strftime('%d/%m/%y %H:%M')} | Falla: {diag} | Fin: {FECHA_HORA_FIN} | Duración: {duracion} | Estatus: {estatus}"
    ...
    if meses:
        ...
        for index, row in df_historial[df_historial['MES_AÑO'] == mes_sel].iterrows():
            ...
            titulo_hist = f"🟢 **Pozo: {row.get('NUM_POZO', 'N/A')}** | Inicio: {row['FECHA_HORA_INICIO'].strftime('%d/%m/%y')} | Falla: {diag} | Fin: {FECHA_HORA_FIN} | Duración: {duracion} | Estatus: {estatus}"
