...
def obtener_rango_fechas(opcion, hoy):
    medianoche = hoy.replace(hour=0, minute=0, second=0, microsecond=0)
    if opcion == "Hoy": return medianoche, hoy
    elif opcion == "Ayer": return (medianoche - dt.timedelta(days=1)), (medianoche - dt.timedelta(seconds=1))
    elif opcion == "Últimos 7 días": return medianoche - dt.timedelta(days=7), hoy
    elif opcion == "Últimos 14 días": return medianoche - dt.timedelta(days=14), hoy
    elif opcion == "Este Mes": return hoy.replace(day=1, hour=0, minute=0, second=0, microsecond=0), hoy
    elif opcion == "Últimos 6 meses": return medianoche - dt.timedelta(days=180), hoy
    return None, None
...
if "ver_grafico" in st.query_params:
    ...
    nombre_mm = st.query_params.get("nombre", "Macromedidor")
    ...
    # UI Cabecera
    st.markdown(f"""
        <style>div[data-testid="column"] {{ padding-top: 0px !important; }} div[data-testid="stVerticalBlock"] {{ gap: 0px !important; }}</style>
        <div style="display: flex; align-items: center; background-color: #0e1117; padding: 10px 20px; border-radius: 10px; border: 1px solid #30363d; margin-bottom: 10px;">
            ...
            <div style="font-size: 12px; color: #c9d1d9; border-left: 2px solid #00FFFF; padding-left: 15px;">
                <b>ID:</b> {tag_a_graficar} | <b>Nombre:</b> {info['Nombre']} | <b>Domicilio:</b> {info['Domicilio']}
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    with col_sel:
        opcion_fecha = st.selectbox("rango", ["Hoy", "Ayer", "Últimos 7 días", "Últimos 14 días", "Este Mes", "Últimos 6 meses", "Personalizado"], index=2, label_visibility="collapsed")

    # Definir f_ini y f_fin
    if opcion_fecha == "Personalizado":
        ...
        f_ini, f_fin = obtener_rango_fechas(opcion_fecha, hoy_dt)

    # Consulta de datos
    query = f"SELECT FECHA, Flujo, Presion, Consumo FROM MACROMEDIDORES WHERE Medidor = '{tag_a_graficar}' AND FECHA BETWEEN '{f_ini.strftime('%Y-%m-%d %H:%M:%S')}' AND '{f_fin.strftime('%Y-%m-%d %H:%M:%S')}' ORDER BY FECHA ASC"
    df = pd.read_sql(query, engine)
    ...
    if not df.empty:
        ...
        df_diario = df.groupby('FECHA_D')['Consumo'].sum().reset_index()
        fig_bar = px.bar(df_diario, x='FECHA_D', y='Consumo', color_discrete_sequence=['#00FFFF'])
