...
# 1. INICIALIZAMOS EL LIENZO Y SUS EJES SIEMPRE, SIN DEPENDER DE LA EXISTENCIA DE DATOS
fig_line = go.Figure()
...
fig_line.update_layout(
    ...
    # Definimos explícitamente los ejes para que no desaparezcan
    ...
    yaxis=dict(title=dict(text="<b>Caudal (Lps)</b>", font=dict(color="#00d4ff")), tickfont=dict(color="#00d4ff"), side="left", anchor="free", position=0.07, showline=True, linecolor='white', linewidth=1.5, autorange=True),
...
# 2. SOLO SI HAY DATOS, AÑADIMOS LAS TRAZAS
if not df.empty:
    for t in tags_grafico:
        ...
        fig_line.add_trace(go.Scatter(
            x=dft_l['FECHA'], y=dft_l['VALUE'], 
            name=t['label'], mode='lines+markers', 
            line=dict(color=t['color'], width=2.2), 
            yaxis=t['axis'], showlegend=True
        ))

    # Dibujo de líneas de tiempo
...
# 3. RENDERIZADO FINAL (SIEMPRE SE MUESTRA EL OBJETO FIGURA)
